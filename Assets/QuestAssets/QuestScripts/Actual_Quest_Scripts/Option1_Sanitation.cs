...
public class Option1_Sanitation : MonoBehaviour
{
        ...
        try
        {
            switch (data.questStep)
            {
                case 0:
                    ...
                    if (!data.isOptional)
                    {
                        QuestBrain.instance.mainQuestState = 6;
                    }

                    //first quest step; this is the default when the quest is first started. go find the next step (usually an item)
                    if (curRoom.roomID == 6 && !Inventory_Brain.instance.inventory.Contains(questItem)) //kitchen, sandwich
                    {
                        CreateInteractPoint(0, 2);
                    }
                    ;
